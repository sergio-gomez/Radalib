...
-- @filename Graphs_Modularities_Test_F.adb
-- @author Sergio Gomez
-- @version 1.0
-- @date 20/03/2006
-- @revision 02/09/2020
...
with Graphs_Float_Modularities; use Graphs_Float_Modularities;
...
procedure Graphs_Modularities_Test_F is
...
begin
  ...
  for Mt in Modularity_Type loop
    Put_Line(Modularity_Type'Image(Mt));
    Put("  Q_Best = "); Put_Float(Q_Best(Mt).Total, Aft => 6);
    Put(" = "); Put_Float(Q_Best(Mt).Reward, Aft => 6);
    Put(" - "); Put_Float(Q_Best(Mt).Penalty, Aft => 6);
    New_Line;
    Put_Line("---");
    Put(Lol_Best(Mt));
    Put_Line("------");
  ...
  Free(Gr);
end Graphs_Modularities_Test_F;
