...
-- @filename Matrix_To_List.adb
-- @author Sergio Gomez
-- @version 1.0
-- @date 14/09/2006
-- @revision 26/02/2016
-- @brief Convert a Matrix to List format
...
begin
  ...
  if Argument_Count = 2 then
    ...
    Put_Line("Usage:  " & Command_Name & " matrix_input_file  list_output_file  [ no_link_string ]");
    New_Line;
    Put_Line("   matrix_input_file : text file containing a matrix");
    New_Line;
    Put_Line("   list_output_file  : name of the output list file");
    Put_Line("                         if the matrix is symmetric, the lower triangular links are discarded");
    New_Line;
  ...
  if Is_Symmetric(Gr) then
    To_Undirected(Gr);
    Put_Line("  Symmetric matrix");
  else
    Put_Line("  Non-symmetric matrix");
  end if;
  ...
  for I in 1..N loop
    ...
    while Has_Next(El) loop
      ...
      Vj := To(E);
      J := Index_Of(Vj);
      if Is_Directed(Gr) or else I <= J then
        Put_Line(F_Out, Get_Name(Vi) & HTab & Get_Name(Vj) & HTab & U2S(Value(E)));
      end if;
    end loop;
  end loop;
  Close(F_Out);
